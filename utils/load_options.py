...
def load_options(default_config, user_config):
    ...
    options['l1_tags'] = [el.strip() for el in
                             default_config.get('database',
                                                'l1_tags').split(',')]
    options['l2_tags'] = [el.strip() for el in
                             default_config.get('database',
                                                'l2_tags').split(',')]
    options['l3_tags'] = [el.strip() for el in
                             default_config.get('database',
                                                'l3_tags').split(',')]
    options['l4_tags'] = [el.strip() for el in
                             default_config.get('database',
                                                'l4_tags').split(',')]
    options['l5_tags'] = [el.strip() for el in
                             default_config.get('database',
                                                'l5_tags').split(',')]
    ...
    options['labels'] = [n for n, m in
                             zip(label_names, labels) if m != 'None']

    options['y_names'] = [n + '.nii.gz' for n, m in
                          zip(label_names, labels) if m != 'None']

    options['modalities'] = [n for n, m in
                             zip(names, modalities) if m != 'None']
    options['label_tags'] = [m for m in labels if m != 'None']
